...
import {
    DEFAULT_SETTINGS,
    AmazonS3UploaderPluginSettings,
    AmazonS3UploaderSettingTab,
} from "settings";
...
export default class AmazonS3UploaderPlugin extends Plugin {
    ...
    async loadSettings() {
        this.settings = Object.assign(
            {},
            DEFAULT_SETTINGS,
            (await this.loadData()) as Partial<AmazonS3UploaderPluginSettings>,
        );
    }
    ...
    registerEvents() {
        this.registerEvent(
            this.app.workspace.on(
                "editor-paste",
                this.uploader.onClipboardAutoUpload.bind(this.uploader),
            ),
        );
        this.registerEvent(
            this.app.workspace.on(
                "editor-drop",
                this.uploader.onDropAutoUpload.bind(this.uploader),
            ),
        );
    }
        ...
        this.registerEvent(
            this.app.workspace.on(
                ...
                (
                    ...
                    // 仅限允许上传的文件类型
                    if (!this.helper.isAllowFileByExt(abstractFile.path))
                        return false;
