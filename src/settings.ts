...
export class AmazonS3UploaderSettingTab extends PluginSettingTab {
    ...
    display(): void {
        ...
        new Setting(containerEl).setName("S3 设置").setHeading();

        new Setting(containerEl).setName("自定义节点").addText((text) =>
            text
                .setPlaceholder(
                    "使用 amazon S3 时可不填，使用第三方兼容 S3 的服务时必填",
                )
        ...
        new Setting(containerEl).setName("访问密钥 ID").addText((text) =>
            text
        ...
        new Setting(containerEl).setName("访问密钥").addText((text) => {
        ...
        new Setting(containerEl).setName("地区").addText((text) =>
            text
        ...
        new Setting(containerEl).setName("桶名").addText((text) =>
            text
        ...
        new Setting(containerEl)
            .setName("上传路径模板")
            .setDesc(
                "上传路径模板, 支持这些占位符: {year}, {month}, {day}, {hour}, {minute}, {second}, {millisecond}, {timestamp}, {timestampMS}, {fullName}, {fileName}, {extName}, {md5}, {sha1}, {sha256}",
            )
            .addText((text) =>
                text
        ...
        new Setting(containerEl)
            .setName("输出 URL 模板")
            .setDesc(
                "输出 URL 模板, 支持这些占位符: {endpoint}, {bucket}, {path}",
            )
            .addText((text) =>
                text
        ...
        new Setting(containerEl)
            ...
            .setDesc(
                "允许上传下载的图片扩展名，用英文逗号分割。不在此列表中的图片将不会被上传或下载",
            )
        ...
        new Setting(containerEl)
            ...
            .setDesc(
                "允许上传下载的文件扩展名，用英文逗号分割。不在此列表中的文件将不会被上传或下载",
            )
            .addTextArea((textArea) =>
                textArea
        ...
        new Setting(containerEl)
            ...
            .addTextArea((textArea) =>
                textArea
                    .setValue(
                        this.plugin.settings.newWorkBlackDomains.join(","),
                    )
                    .onChange(async (value) => {
        ...
        new Setting(containerEl)
            ...
            .setDesc(
                "当你复制时，某些应用会在剪切板中同时写入文本和文件数据，确认是否上传。",
            )
            .addToggle((toggle) =>
                toggle
        ...
        new Setting(containerEl)
            .setName("拖拽自动上传")
            .setDesc(
                "启用该选项后，拖拽文件时会自动上传。如果按住 ctrl/cmd，将放行并执行 Obsidian 默认行为（保存到本地）。",
            )
            .addToggle((toggle) =>
                toggle
        ...
        new Setting(containerEl)
            .setName("文件下载代理")
            .setDesc(
                "设置文件下载的代理地址，为空表示不使用代理。使用 {url} 占位符代表下载链接，例如：http://localhost:7890/proxy?url={url}",
            )
            .addText((text) =>
                text
        ...
        new Setting(containerEl)
            .setName("Referer 规则")
            .setDesc(
                "设置 Referer 规则，用于防盗链下载。一行一个规则, 格式为 {domain},{referer}，例如：example.com,https://example.com\nexample.org,https://example.org",
            )
            .addTextArea((textArea) =>
                textArea
                    .setValue(
